...
for i=1:length(annot)
    for j=1:length(TIME_M)
        ...
        [c,t]=max(TIME_M(2,(j-5):(j+45)));
        R_peak(1,k)=TIME_M(1,(j-6+t));
        R_peak(2,k)=c;
       index(k)=j-6+t;%峰值点在TIME_M中的序号
       k=k+1;
        end
        continue;
    end
...
newf=strcat('D:\ECG\ECG资料\ex201705212012\channel2\',NUM,'\data\');
mkdir(newf);
 for i=1:(length(R_peak)-2)
savefile=strcat('A',num2str(i));
file=strcat(newf,['A',num2str(i),'_',NUM,'_2'])
save(file,savefile);
end
  
newf2=strcat('D:\ECG\ECG资料\ex201705212012\channel2\',NUM,'\images\');
mkdir(newf2);
for i=1:(length(R_peak)-2)
    Pitdata=strcat(newf,'A',num2str(i),'_',NUM,'_2','.mat')
   ...
   imagename=strcat(newf2,['A',num2str(i)],'_',NUM,'_2','.jpg')
   %myFileName=['A',num2str(i),'.jpg']
   print('-djpeg',imagename,'-r100');
end

annot(:,length(annot))=[];annot(:,1)=[];
annotation=strcat('D:\ECG\ECG资料\ex201705212012\channel2\',NUM,'\','annotation',NUM,'_2');
annota=strcat('annot');
...
newf=strcat('D:\ECG\ECG资料\ex201705212012\channel2_single\',NUM,'\data\');
mkdir(newf);
 for i=1:(length(R_peak)-2)
savefile=strcat('A',num2str(i));
file=strcat(newf,['A',num2str(i),'_',NUM,'_2'])
...
newf2=strcat('D:\ECG\ECG资料\ex201705212012\channel2_single\',NUM,'\images\');
mkdir(newf2);
for i=1:(length(R_peak)-2)
    Pitdata=strcat(newf,'A',num2str(i),'_',NUM,'_2','.mat')
    f=cell2mat(struct2cell(load(Pitdata)));
...
annot(:,length(annot))=[];annot(:,1)=[];
annotation=strcat('D:\ECG\ECG资料\ex201705212012\channel2_single\',NUM,'\','annotation',NUM,'_2');
annota=strcat('annot');
save(annotation,annota);
