...
intermediate_tensor_function = K.function([model.layers[0].input],[model.layers[13].output])
intermediate_tensor = intermediate_tensor_function([250,250,1])[0]

#model.compile(loss=keras.losses.categorical_crossentropy,
 #             optimizer=keras.optimizers.Adadelta(),
 #             metrics=['accuracy'])
#dense1_layer_model = Model(inputs=model.input,outputs=model.get_layer('Dense_2').output)  
#dense1_output = dense1_layer_model.predict(x_test, y_test,verbose=0)  
